...
import { LoaderCircle, Mail, Send, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function Contact() {
  ...
  const [state, setState] = useState({ loading: false, type: "", message: "" });
  ...
  const handleSubmit = async (event) => {
    ...
    setState({ loading: true, type: "", message: "" });

    try {
      const result = await sendContactMessage(form);
      setState({ loading: false, type: "success", message: result.message });
      setForm(initialForm);
    } catch (error) {
      setState({
        loading: false,
      ...
      <div className="section-shell">
        ...
        <div className="grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          >
            ...
            <h3 className="mt-6 text-2xl font-bold text-white">Start a conversation</h3>
            <p className="mt-3 leading-7 text-zinc-400">
              I'm open to connecting with professionals, founders, recruiters and organizations interested in AI automation, software development and digital transformation.
            </p>
            <a href={`mailto:${site.email}`} className="mt-7 inline-block text-sm font-semibold text-white underline decoration-white/20 underline-offset-4">
              {site.email}
            </a>
          </motion.div>
          ...
          >
            ...
            <div className="mt-4">
              <Field label="Subject" name="subject" value={form.subject} onChange={handleChange} placeholder="What would you like to build?" />
            </div>
            <div className="mt-4">
              <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.15em] text-zinc-500">Message</label>
              <textarea
                required
                name="message"
                value={form.message}
            ...
            {state.message && (
              <div className={`mt-4 flex items-center gap-2 rounded-xl border p-3 text-sm ${
                ...
                {state.type === "success" ? <CheckCircle2 size={16} /> : <AlertCircle size={16} />}
                {state.message}
              </div>
            )}
            ...
            >
              {state.loading ? <LoaderCircle className="animate-spin" size={17} /> : <Send size={17} />}
...
function Field({ label, name, type = "text", value, onChange, placeholder }) {
  return (
    <>
      <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.15em] text-zinc-500">{label}</label>
      <input
        required
        type={type}
        name={name}
